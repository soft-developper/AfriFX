#!/usr/bin/env bash
# ============================================================================
# Phase 5c - Bridge self-heal: add missing chains on demand
#
# WHY
#   The bridge failed for accounts created BEFORE Phase 5a with "Transfer not
#   started / Lost track of the transaction". Cause: 5a only provisions the
#   CCTP bridge chains at sign-up, so an older wallet exists only on Arc. When
#   the bridge tried to approve/burn on the source chain, the server couldn't
#   find a wallet there and the client's poll error-stormed into "Lost track".
#
# WHAT IT DOES
#   Makes executeContractCall SELF-HEAL: when the server reports the wallet
#   isn't on the target chain (NEEDS_CHAIN), it adds the chains (one approval),
#   waits for Circle to index them, and retries the call once. This fixes every
#   account - old or new - with no separate migration.
#   Also hardens two server bits:
#     * find-contract returns PENDING (not a 502) while a just-added chain is
#       still indexing, so the poll loop waits instead of failing.
#     * findContractExecution uses a minimal Circle query (wallet only) and
#       matches blockchain + contract + recency in code, avoiding a 400 from
#       an over-narrow server-side filter.
#
# CHANGES (3 files)
#   afrifx-web/hooks/useCircleTx.ts        + prepareContractCall, ensureBridgeChains, retry
#   afrifx-api/src/routes/auth.ts          find-contract: PENDING on NEEDS_CHAIN
#   afrifx-api/src/services/circleWallets.ts  findContractExecution: minimal query
#
# REQUIRES: Phase 5b already applied.
#
# USAGE
#   bash phase5c-bridge-selfheal.sh
# ============================================================================
set -euo pipefail
...
grep -q "executeContractCall" "$WEB/hooks/useCircleTx.ts" || die "Phase 5b not detected. Apply 5b first."

BK="$ROOT/.phase5c-backup"; rm -rf "$BK"; mkdir -p "$BK"
...
ok "snapshot saved to .phase5c-backup"
say "Writing files"
cat > "$ROOT/afrifx-web/hooks/useCircleTx.ts" <<'AFX_5C_EOF'
'use client'
...
cat > "$ROOT/afrifx-api/src/routes/auth.ts" <<'AFX_5C_EOF'
/**
...
cat > "$ROOT/afrifx-api/src/services/circleWallets.ts" <<'AFX_5C_EOF'
/**
...
say "Verifying"
grep -q "ensureBridgeChains" "$WEB/hooks/useCircleTx.ts" || die "self-heal missing"
grep -q "prepareContractCall" "$WEB/hooks/useCircleTx.ts" || die "prepare helper missing"

say "API: install + tsc + tests"
( cd "$API" && npm install --no-audit --no-fund --loglevel=error >/dev/null 2>&1 && npx tsc --noEmit ) \
  || { cp -rf "$BK"/. "$ROOT"/; die "API tsc failed - restored"; }
ok "api typechecks"
...
( cd "$WEB" && npm install --no-audit --no-fund --loglevel=error >/dev/null 2>&1 && npx tsc --noEmit ) \
  || { cp -rf "$BK"/. "$ROOT"/; die "web tsc failed - restored"; }
ok "web typechecks"
( cd "$WEB" && rm -rf .next && npm run build >/dev/null 2>&1 ) || { cp -rf "$BK"/. "$ROOT"/; die "web build failed - restored"; }
ok "web builds"

rm -rf "$BK"
say "Phase 5c applied and verified."
cat <<'NOTE'

  Fixed: bridge now self-heals for accounts created before Phase 5a. On the
  first bridge, if the wallet isn't on the source/destination chain yet, it
  adds the chains (one extra approval), waits, and continues.

  Commit & push (auto-deploys via CI -> Vercel + Render):
    git add afrifx-web/hooks/useCircleTx.ts \
            afrifx-api/src/routes/auth.ts \
            afrifx-api/src/services/circleWallets.ts
    git commit -m "phase 5c: bridge self-heals missing wallet chains on demand"
    git push origin main

  Then retry the bridge on your existing account:
    - Start a small Base -> Arc bridge.
    - You'll now see an extra "enable this network" approval the first time.
    - Approve it, then the approve/burn/mint prompts follow as normal.
    - If it still fails, open devtools > Network, reproduce, and tell me what
      /auth/wallet/tx/contract returns (status + body).
NOTE
