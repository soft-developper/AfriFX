#!/usr/bin/env bash
# ============================================================
# phase0-dispute-duty-fix.sh
#
# PHASE 0.1 - Close the dispute duty bypass.
#
# Three holes, all letting an OFF-DUTY sub-admin settle a disputed
# trade and move real funds:
#
#   1. POST /admin/manage/offers/:id/release
#   2. POST /admin/manage/offers/:id/cancel
#        Gated by manage_offers only. No duty check, no check that
#        the offer is under dispute. The offers list returned
#        disputed offers, so both were reachable from the UI.
#
#   3. POST /admin/manage/disputes/:id/resolve
#        Gated by resolve_disputes only. No duty check and no
#        assignment check, so any admin with the permission could
#        settle ANY dispute by id without ever accepting it.
#
# Fixes:
#   - offers force actions reject any offer with an OPEN dispute (409)
#   - dispute resolve now requires on-duty AND being the assignee
#   - GET /offers returns { offers, disputedIds } so the UI can flag
#     disputed rows and hide the force buttons
#   - blocked attempts are written to the audit log
#
# IMPORTANT: raising a dispute does NOT set p2p_offers.status to
# 'disputed' - the offer stays 'accepted' and the dispute lives in the
# `disputes` table. A status-based guard would silently never fire, so
# the guard queries for an open dispute row instead.
#
...
mkdir -p "$(dirname "afrifx-api/src/routes/adminManage.ts")"
cat > 'afrifx-api/src/routes/adminManage.ts' <<'AFX_ADMINMANAGE_EOF'
...
// DISPUTE GUARD
//
// An offer under dispute must ONLY be settled through the dispute
// section, which enforces the duty session (accept -> assigned admin
// -> resolve). The force-release/force-cancel actions below move real
// funds and are gated by manage_offers alone, so without this guard a
// sub-admin who is off duty can settle a disputed trade from the
// offers page and bypass the whole duty system.
//
// NOTE: raising a dispute does NOT change p2p_offers.status - the
// offer stays 'accepted' and the dispute lives in the `disputes`
// table. Checking the offer status would silently never match, so we
// look for an open dispute row instead.
// ══════════════════════════════════════════════════════════
async function openDisputeFor(offerId: string): Promise<string | null> {
  const rows = parseRows(await db.run(
    sql`SELECT id FROM disputes WHERE offer_id = ${offerId} AND status = 'open' LIMIT 1`
  ))
...
router.get('/offers', requirePermission(PERMISSIONS.MANAGE_OFFERS), async (req, res) => {
  ...
  try {
    ...
    // admins should still SEE a disputed offer here, they just can't act
    // on it from this page. Sent as a separate id list so the row shape
    // (which may be a positional array) is left untouched.
    const openRows = parseRows(await db.run(
      sql`SELECT offer_id FROM disputes WHERE status = 'open'`
    ))
    const disputedIds = openRows.map(
      (r: any) => String(Array.isArray(r) ? r[0] : r.offer_id)
    )

    res.json({ offers, disputedIds })
  } catch (err: any) { res.status(500).json({ error: err.message }) }
})
...
router.post('/offers/:id/release', requirePermission(PERMISSIONS.MANAGE_OFFERS), async (req, res) => {
  ...
  try {
    const disputeId = await openDisputeFor(offerId)
    if (disputeId) {
      await logAction(admin.id, admin.username, 'blocked_force_release', 'offer', offerId,
        `Blocked force release: offer is under open dispute ${disputeId}`, req.ip).catch(() => {})
      return res.status(409).json({
        error: 'This offer is under dispute. Settle it from the Disputes section, where duty rules apply.',
        code:  'offer_under_dispute',
        disputeId,
      })
    }
...
router.post('/offers/:id/cancel', requirePermission(PERMISSIONS.MANAGE_OFFERS), async (req, res) => {
  ...
  try {
    const disputeId = await openDisputeFor(offerId)
    if (disputeId) {
      await logAction(admin.id, admin.username, 'blocked_force_cancel', 'offer', offerId,
        `Blocked force cancel: offer is under open dispute ${disputeId}`, req.ip).catch(() => {})
      return res.status(409).json({
        error: 'This offer is under dispute. Settle it from the Disputes section, where duty rules apply.',
        code:  'offer_under_dispute',
        disputeId,
...
router.post('/disputes/:id/resolve', requirePermission(PERMISSIONS.RESOLVE_DISPUTES), async (req, res) => {
  ...
  try {
    // ── Duty + assignment gate ─────────────────────────────
    // Accepting a dispute is duty-gated, but resolving it was not, so an
    // off-duty sub-admin could settle any dispute directly by id without
    // ever accepting it. Mirror the gate from POST /disputes/:id/accept:
    // the admin must be on duty AND be the one this dispute is assigned
    // to. Super admins bypass both, as they do elsewhere.
    if (admin.role !== 'super_admin') {
      const { isOnDuty } = await import('../lib/duty')
      const duty = await isOnDuty(admin.id)
      if (!duty.onDuty) {
        return res.status(403).json({
          error: duty.reason ?? 'You must be on duty to resolve a dispute',
          code:  'not_on_duty',
        })
      }

      const assignRows = parseRows(await db.run(
        sql`SELECT admin_id FROM dispute_assignments WHERE dispute_id = ${req.params.id} LIMIT 1`
      ))
      if (!assignRows.length) {
        return res.status(409).json({
          error: 'Accept this dispute before resolving it.',
          code:  'dispute_not_accepted',
        })
      }
      const a  = assignRows[0]
      const to = String(Array.isArray(a) ? a[0] : a.admin_id)
      if (to !== admin.id) {
        return res.status(403).json({
          error: 'This dispute is assigned to another admin.',
          code:  'not_assignee',
        })
      }
    }
...
mkdir -p "$(dirname "afrifx-web/app/admin/offers/page.tsx")"
cat > 'afrifx-web/app/admin/offers/page.tsx' <<'AFX_OFFERSPAGE_EOF'
'use client'
...
echo ""
echo "→ Done. No migration needed for this change."
echo ""
