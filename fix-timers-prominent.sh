#!/bin/bash
# Run from ~/AfriFX:  bash fix-timers-prominent.sh
set -e
echo "⏱  Making timers prominent + dynamic color..."

# ============================================================
# 1 — Updated useCountdown — adds percentage elapsed
# ============================================================
...
__EOF__
echo "✅  useCountdown — pctElapsed + isWarning + isDanger"

# ============================================================
# 2 — New prominent TimerBanner component
# ============================================================
cat > afrifx-web/components/p2p/TimerBanner.tsx << '__EOF__'
'use client'
...
__EOF__
echo "✅  components/p2p/TimerBanner.tsx — large prominent timer"

# ============================================================
# 3 — Old CountdownTimer — keep for small inline uses
# ============================================================
...
__EOF__
echo "✅  CountdownTimer — updated with dynamic color"

# ============================================================
# 4 — Updated offer detail page — TimerBanner at top
# ============================================================
...
export default function OfferDetailPage() {
  ...
  async function handleDispute() {
    ...
    setDisputing(true)
    try {
      await raiseDispute(offer!.id, 'Maker did not confirm receipt within the agreed window')
      setDisputeDone(true)
      await load()
  ...
  return (
    <div>
      ...
      {/* ── PROMINENT TIMER BANNER — full width, above everything ── */}
      <ClientOnly>
        {showTakerTimer && (
          <div className="mb-4">
            <TimerBanner
        ...
        <div className="rounded-xl border border-[#1B2B4B] bg-[#0F1729] p-5">
          ...
          <div className="space-y-2 text-xs">
            {[
              ...
              ['Taker window', `${timerSecs >= 3600 ? timerSecs/3600 + 'h' : timerSecs/60 + 'min'} (resets for maker after taker confirms)`],
            ].map(([l,v]) => (
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="mt-0.5 text-amber-600">
                        USDC locked. Auto-releases to taker in 24h if unresolved.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <>
                  ...
                  {isMaker && offer.taker_confirmed && !offer.maker_confirmed && (
                    <div className="rounded-lg border border-[#378ADD]/30 bg-[#378ADD]/10 p-3 text-xs">
                      ...
                      <p className="mt-1 text-[#64748B]">
                        Taker confirmed sending <strong className="text-[#E2E8F0]">
                          {Number(offer.local_amount).toLocaleString()} {offer.local_currency}
                  ...
                  {/* Dispute button — appears when maker timer expired */}
                  ...
                  {!isInvolved && (
                    <p className="text-center text-xs text-[#64748B]">Trade in progress.</p>
                  )}
                </>
              )}
            </div>
...
__EOF__
echo "✅  marketplace/[id]/page.tsx — TimerBanner above content"

echo ""
echo "══════════════════════════════════════════════════════"
echo "✅  Timer improvements complete!"
echo ""
echo "  Timer now shows as a full-width banner above the"
echo "  offer summary and progress cards with:"
echo ""
echo "  Color stages (based on % of time elapsed):"
echo "    0–70%  → Blue  (normal)   — plenty of time"
echo "    70–90% → Amber (warning)  — running low"
echo "    90–100%→ Red   (danger)   — act now"
echo "    100%   → Red   (expired)  — time up"
echo ""
echo "  Features:"
echo "    • Large 4xl countdown font (tabular numbers)"
echo "    • Depleting progress bar"
echo "    • '% elapsed' label"
echo "    • Phase label (whose turn + what they need to do)"
echo "    • Resets when phase switches (taker → maker)"
echo ""
