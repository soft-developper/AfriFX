...
mkdir -p "afrifx-web/hooks"
cat > "afrifx-web/hooks/useIsAdmin.ts" << 'ISADMIN_EOF'
'use client'
...
ISADMIN_EOF
echo "  hooks/useIsAdmin.ts (was calling a removed wallet-based endpoint)"
...
SHELL_EOF
echo "  components/admin/AdminShell.tsx (mobile off-canvas nav + Settings link)"
...
echo ""
echo "  Also updated:"
echo "    hooks/useIsAdmin.ts -- was calling /admin/auth/is-admin?wallet=..,"
echo "       an endpoint Phase 12 removed entirely. It now checks for a"
echo "       valid admin session instead, so the 'Admin panel' link in the"
echo "       general platform's Sidebar and MobileDrawer works again."
echo "    components/admin/AdminShell.tsx -- had no mobile breakpoint at"
echo "       all (fixed 224px sidebar, no collapse). It now mirrors the"
echo "       general platform's off-canvas drawer pattern on small screens."
