  ...
  return (
    <div className="space-y-2 text-xs">
      <div className="flex items-center justify-between">
        <span className="text-app-muted">Seller (receives local)</span>
        <UserDisplay
      ...
      <div className="flex items-center justify-between">
        <span className="text-app-muted">Buyer (receives USDC)</span>
        {takerAddress
          ? <UserDisplay address={takerAddress} size="xs" suffix={isTaker ? '(you)' : undefined} />
          : <span className="text-app-muted text-xs">Waiting for buyer…</span>
        }
      </div>
    </div>
  )
