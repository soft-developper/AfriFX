#!/usr/bin/env bash
# ============================================================
# fix-google-redirect.sh
#
# FIX: after signing in with Google you land back on the LANDING PAGE
# instead of the dashboard.
#
# CAUSE
# The Google redirectUri was window.location.origin, copied from
# Circle's tutorial where the demo app IS the root page. In AfriFX the
# root is the marketing landing page. So Google returned the browser to
# "/", where the Circle SDK callback that finishes the handshake does
# not exist - the login was silently dropped and you saw the landing
# page again.
#
# FIX
# redirectUri now points at ${origin}/signin, the only page that
# registers the callback.
#
# ⚠ YOU MUST ALSO UPDATE GOOGLE CLOUD CONSOLE
# Add the new URI under APIs & Services -> Credentials -> your OAuth
# client -> Authorized redirect URIs. It must match EXACTLY:
#
#     https://<your-vercel-domain>/signin
#     http://localhost:3000/signin        (for local dev)
#
# Keep the old bare-origin entries or remove them, either is fine, but
# the /signin ones must be present or Google will reject the redirect
# with redirect_uri_mismatch.
#
# ALSO: visiting /signin while already signed in now goes straight to
# the dashboard instead of asking again.
#
...
cat > 'afrifx-web/lib/circle.ts' <<'AFX_N00_EOF'
'use client'
...
cat > 'afrifx-web/app/(auth)/signin/page.tsx' <<'AFX_N01_EOF'
'use client'
...
echo "→ Done."
echo ""
echo "⚠ ADD THESE TO GOOGLE CLOUD CONSOLE (Authorized redirect URIs):"
echo "     https://<your-vercel-domain>/signin"
echo "     http://localhost:3000/signin"
echo "   Without them Google will fail with redirect_uri_mismatch."
echo ""
