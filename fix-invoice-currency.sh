#!/bin/bash
# Run from ~/AfriFX:  bash fix-invoice-currency.sh
set -e
echo "🔧  Fixing invoice currency conversion..."
...
__EOF__
echo "✅  pay/[ref]/page.tsx — local currency → USDC conversion added"

echo ""
echo "══════════════════════════════════════════════════════"
echo "✅  Invoice currency conversion fixed!"
echo ""
echo "  Root cause:"
echo "  pay page used invoice.amount directly as USDC amount"
echo "  10,000 NGN → tried to send 10,000 USDC → balance too"
echo "  low → wallet can't estimate gas → 'unavailable'"
echo ""
echo "  Fix:"
echo "  Invoice NGN/GHS/KES/ZAR/EGP → converted to USDC first"
echo "  using live rate: usdcAmount = localAmount / rate"
echo ""
echo "  10,000 NGN ÷ 1,372 = 7.29 USDC → correct amount sent"
echo ""
echo "  The pay button now shows:"
echo "  'Pay 7.2921 USDC (≈ 10,000 NGN)'"
echo ""
echo "  Also shows:"
echo "  • USDC conversion amount prominently on the page"
echo "  • Live rate used (1 USDC = X NGN)"
echo "  • Waits for rates to load before enabling Pay button"
echo "══════════════════════════════════════════════════════"
